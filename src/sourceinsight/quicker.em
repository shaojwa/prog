...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 2,  "  Copyright (C), 2010-2011, GBCOM Co., Ltd.")
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  版权所有 (C), 2010-2012, 上海寰创通信科技有限公司")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    InsBufLine(hbuf, ln+6, " 最近一次修改记录  : ")
    if( strlen(szMyName)>0 )
    {
       InsBufLine(hbuf, ln+7, " 修改作者  : @szMyName@")
    }
    else
    {
       InsBufLine(hbuf, ln+7, " 修改作者  : #")
    }
    InsBufLine(hbuf, ln+8, " 修改目的  :")
    SysTime = GetSysTime(1);
    szTime = SysTime.Date

    InsBufLine(hbuf, ln+9, " 修改日期  : @szTime@") 
    InsBufLine(hbuf, ln+10, "*****************************************************************************/")
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        InsBufLine(hbuf, ln+11, "UINT32_T  @szFunc@( # )")
    ...
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        ...
        if(strlen(szRet) > 0)
        {
            ...
            SetbufIns(hbuf,ln+11,strlen(szRet)+strlen(szFunc) + 3
        }
        szFuncDef = ""
