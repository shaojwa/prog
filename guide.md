...
## 格式
#### 行长度
- 每一行代码字符数不超过100字符，命令行示例或者`URL`除外。
...
变量命名可以不用考虑带上类型，比如`interval_set cached_intervals;`
没什么必要，所有字段全部表意就好，比如`cached_segs;`或者`cached_ranges`。
...
#### 返回值用0还是定义宏
我个人返回宏定义，尽管很多项目都这么用，linux标准错误码中没有0，非0值才有专门的宏定义。
所以0就是唯一的那个在任何情况下都表示成功的值，不需要定义各种表示成功的宏。

## 性能相关
- 频繁调用接口多用内联。
- 考虑使用task的栈变量，而不是堆变量，以减少内存分配和释放。
- 考虑使用侵入式容器，以减少内存分配和释放。
- 使用引用或指针传参以减少拷贝， POD类型不需要因为性能而使用引用或指针。
...
日志非常重要，日志是问题定位的探针，要用最少的日志，覆盖最全的信息，日志要比代码更加精炼。
